...
export const getBlockTitle = title => {
  ...
  if (title.includes('egg-stories'))
    return 'Stories from Educators Going Global';
};
...
export const filteredList = (cards, tag, query, route, home, pageTitle) =>
  cards
    .filter(
      card => {
        const contentTag =
          getContentTag(route, pageTitle) ||
          ...
          if (
            !card.metadata.tags.find(tagObj => {
              ...
              return (
                ...
                toKebabCase(contentTag).indexOf(toKebabCase(tagObj.sys.id)) > -1
              );
            })
          )
            return false;
